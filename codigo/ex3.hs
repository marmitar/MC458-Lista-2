...
minMax :: Tree -> Maybe (Float, Float)
minMax Empty = Nothing
minMax (Node _ a b c) = (minMax a) `joinR` (minMax b) `joinR` (minMax c)
    where joinR res Nothing = res
          joinR Nothing res = res
          joinR (Just (la, ha)) (Just (lb, hb)) = Just (min la lb, max ha hb)
...
tuscaTst :: Float -> Tree -> Tree -> Tree -> (Bool, Float, Float)
tuscaTst r te tc td = (ca && cb && cc, vme, vmd)
    where tst Empty _ _ = (True, r, r)
          tst (Node xr xte xtc xtd) conde condd = (adt && conde vme && condd vmd, vme, vmd)
            where (adt, vme, vmd) = tuscaTst xr xte xtc xtd
          (ca, vme, _) = tst te (const True) ((>) r)
          (cb, _, _) = tst tc ((==) r) ((==) r)
          (cc, _, vmd) = tst td ((<) r) (const True)
...
eTusca :: Tree -> Bool
eTusca Empty = True
eTusca (Node r e c d) = adt
    where (adt, _, _) = tuscaTst r e c d
